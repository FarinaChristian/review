...
def heart_rate_fft2d(signals, Fs=25):
    #rimuovo il valore medio prima della 2D FFT
    ...
    valid = (fx >= 0.8) & (fx <= 2) #cerco il picco, non su tutto lo spettro, ma all'interno della banda biologica

    mag_valid = mag[:, valid]
    fx_valid = fx[valid]

    _, ix = np.unravel_index(np.argmax(mag_valid), mag_valid.shape)
...
def estimate_breath_rate(data,antenna):
    ...
    #così sto separanado fase e ampiezza
    #la magnitudine così mi permette di distinguere oggetti fermi da quelli mobili (questi servono a noi)
    mag = np.abs(trans)
    mag = np.diff(mag, axis=0)
    mag[:, 0] = 0
    arr = mag.mean(axis=0)  
...
def main():
    decoder = AWR1243()
    #path = "C:/Users/lucam/OneDrive/Desktop/univr ing/tirocinio/christian2/*"
    path = "C:/Users/lucam/OneDrive/Desktop/univr ing/tirocinio/christian-muhamad-yosef-vasile/*"
    #path = "C:/Users/lucam/OneDrive/Desktop/univr ing/tirocinio/christian-yosef-muhamad-daniele/*"
